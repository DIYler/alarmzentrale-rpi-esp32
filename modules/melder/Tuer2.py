...
### Abfragescript für Melder Tuer2: Kellertür an ESP32 ###

## Der Türkontakt ist per GPIO an einem ESP32 angeschlossen bzw. Der ESP32 stellt ein API-Endpoint bereit,
## um den Zustand des GPIO abzufragen
## Informationen zum Abfragen:
##
## API-URL http://192.168.0.101/api/g4
...
def GibZustand():
  ...
  try:
    response = requests.get('http://192.168.0.101/api/g4', timeout=5)
    zustand = response.json()['zustand']
    # Störung zurücksetzen falls vorhanden
    loescheStoerung.loescheStoerung("Tuer2")
  except:
    ...
    try:
      ...
      response = requests.get('http://192.16.0.101/api/g4', timeout=5) 
      zustand = response.json()['zustand']
      # Störung zusrücksetzen falls vorhanden
      loescheStoerung.loescheStoerung("Tuer2")
    except:
      # Immer noch kein Lebenszeichen. Störung melden
      setzeStoerung.setzeStoerung("Tuer2")
